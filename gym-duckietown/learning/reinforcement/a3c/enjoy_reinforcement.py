...
def preprocess_state(obs):
    ...
    x = obs.transpose(1, 2, 0).mean(2)
    x = np.uint8(x * 255)
    x = Image.fromarray(x).resize((80, 80))
    x = np.array(x, dtype=np.float32).reshape(1, 80, 80) / 255
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-dir', type=str, default='reinforcement/a3c/models')  # Name of the directory where the models are saved
    parser.add_argument('--model-file', type=str,
                        default='a3c-duckie.pth')  # Name of the model file
